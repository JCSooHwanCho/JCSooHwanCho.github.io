...
* 필요성
    ...
    프로토콜로 타입 반환을 받으면, 해당 타입에 대한 정보가 날아가고, 프로토콜의 일부 인터페이스 만 이용할 수 있게 되는 문제가 생기게 됩니다. Shape에 Equatable을 추가할라고 쳐도 타입 정보가 모두 날아갔기 때문에, 자기 자신의 타입 정보가 필요한 == 메소드를 제대로 호출할 수 조차 없습니다.  
    ...
    이는 프로토콜 타입으로 반환한 값은 해당 프로토콜을 채택하지 않은 것으로 여겨지기 때문입니다. 이는 타입 정보가 날아간 것의 부작용입니다. 
